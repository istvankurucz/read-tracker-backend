...
export default async function updateUserMW(_: Request, res: Response, next: NextFunction) {
	// Get user data
	const { user, userData } = res.locals as { user: User; userData: UpdateUserData };
	...
	try {
		// Update user
		const { deletedAt, ...updatedUser } = await updateUser(user.id, userData);

		// Add user to res.locals
		(res.locals.user as User) = updatedUser;
